...
class VacancyTemplateRowPayload(BaseModel):
    nivel: str
    lotacao: int
    capacidade: int
    professor: str
...
class VacancyTemplateBlockPayload(BaseModel):
    periodoLabel: str
    horario: str
    lotacaoHorario: int
    ...
    excesso: int
    rows: List[VacancyTemplateRowPayload] = Field(default_factory=list)


class VacancyTemplateSummaryPayload(BaseModel):
...
class VacancyTemplateExportPayload(BaseModel):
    blocks: List[VacancyTemplateBlockPayload] = Field(default_factory=list)
    summary: VacancyTemplateSummaryPayload = Field(default_factory=VacancyTemplateSummaryPayload)
...
def _clear_vacancy_block_cells(ws, row_start: int, col_start: int) -> None:
    for row in range(row_start, row_start + 5):
        for col in range(col_start, col_start + 4):
            ws.cell(row=row, column=col).value = ""


def _fill_vacancy_block_cells(ws, row_start: int, col_start: int, block: VacancyTemplateBlockPayload) -> None:
    ws.cell(row=row_start, column=col_start).value = str(block.horario or "").strip()
    ws.cell(row=row_start, column=col_start + 1).value = str(block.periodoLabel or "").strip()

    detail_rows = list(block.rows or [])
    first = detail_rows[0] if len(detail_rows) > 0 else None
    second = detail_rows[1] if len(detail_rows) > 1 else None

    if first:
        ws.cell(row=row_start + 1, column=col_start).value = f"{str(first.nivel or '').strip()}:"
        ws.cell(row=row_start + 1, column=col_start + 1).value = f"{first.lotacao}/{first.capacidade}"
        ws.cell(row=row_start + 1, column=col_start + 2).value = str(first.professor or "").strip()

    if second:
        ws.cell(row=row_start + 2, column=col_start).value = f"{str(second.nivel or '').strip()}:"
        ws.cell(row=row_start + 2, column=col_start + 1).value = f"{second.lotacao}/{second.capacidade}"
        ws.cell(row=row_start + 2, column=col_start + 2).value = str(second.professor or "").strip()

    if len(detail_rows) > 2:
        extras = " | ".join(
            [f"{str(item.nivel or '').strip()} {item.lotacao}/{item.capacidade}" for item in detail_rows[2:]]
        )
        ws.cell(row=row_start + 2, column=col_start + 2).value = extras

    ws.cell(row=row_start + 3, column=col_start).value = "Lotação:"
    ws.cell(row=row_start + 3, column=col_start + 1).value = f"{block.lotacaoHorario}/{block.capacidadeHorario}"

    ws.cell(row=row_start + 4, column=col_start).value = "Vagas:"
    ws.cell(row=row_start + 4, column=col_start + 1).value = str(block.vagasDisponiveis)
    ws.cell(row=row_start + 4, column=col_start + 2).value = "Excesso:"
    ws.cell(row=row_start + 4, column=col_start + 3).value = str(block.excesso)


def _populate_vacancy_template_sheet(ws, blocks: List[VacancyTemplateBlockPayload], summary: VacancyTemplateSummaryPayload, generated_at: str) -> None:
    ws["A1"] = "Relatório de Vagas"
    ws["A2"] = f"Gerado em {generated_at}"
    ws["A3"] = (
        f"Totais: Lotação {summary.totalLotacao}/{summary.totalCapacidade} | "
        f"Vagas {summary.totalVagas} | Excesso {summary.totalExcesso}"
    )

    col_starts = [1, 6, 11]  # A, F, K
    row_starts = [5, 11, 17, 23, 29]

    slot_count = len(col_starts) * len(row_starts)
    for slot in range(slot_count):
        row_idx = slot // len(col_starts)
        col_idx = slot % len(col_starts)
        _clear_vacancy_block_cells(ws, row_starts[row_idx], col_starts[col_idx])

    for slot, block in enumerate(blocks[:slot_count]):
        row_idx = slot // len(col_starts)
        col_idx = slot % len(col_starts)
        _fill_vacancy_block_cells(ws, row_starts[row_idx], col_starts[col_idx], block)


@app.post("/reports/vacancies-template-file")
def generate_vacancies_template_file(payload: VacancyTemplateExportPayload):
    ...
    blocks_per_sheet = 15  # 5 linhas de blocos x 3 colunas de blocos
    chunks = [
        payload.blocks[index:index + blocks_per_sheet]
        for index in range(0, max(len(payload.blocks), 1), blocks_per_sheet)
    ]
    if not chunks:
        chunks = [[]]

    generated_at = str(payload.generatedAt or "").strip() or datetime.now().strftime("%d/%m/%Y, %H:%M:%S")

    first_sheet = True
    sheets = []
    for chunk_index, chunk in enumerate(chunks, start=1):
        ws = template_ws if first_sheet else workbook.copy_worksheet(template_ws)
        first_sheet = False
        ws.title = f"Vagas_{chunk_index}" if len(chunks) > 1 else "Vagas"
        _populate_vacancy_template_sheet(ws, chunk, payload.summary, generated_at)
        sheets.append(ws)
